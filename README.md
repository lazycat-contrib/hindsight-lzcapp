...
- 包名：`community.lazycat.app.hindsight`（0.10.0，含启动修复：超时窗口放宽）
...
**启动修复**：`HINDSIGHT_API_SKIP_LLM_VERIFICATION=true` 去掉启动期 LLM 验证（可阻塞 35s+，失败带重试退避，会拖垮 300s 启动窗口导致重启循环）；`HINDSIGHT_API_MODEL_INIT_TIMEOUT=900` + `HINDSIGHT_API_STARTUP_WAIT_SECONDS=1200` 放宽首次启动（模型加载+迁移+pg0 初始化）（pg0 初始化 + 迁移 + 模型加载）的等待窗口。
